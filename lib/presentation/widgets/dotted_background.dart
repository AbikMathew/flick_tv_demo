...
class _HalftoneDotsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = const Color(0xFFC0A040).withOpacity(0.2) // Goldish dots
    ...
    for (int r = 0; r < rows; r++) {
      ...
      for (int c = 0; c < cols; c++) {
        ...
        final double radialFactor = math.max(0.0, 1.0 - (distanceToCenterTop / (size.width * 0.9)));
        
        final double opacity = 0.25 * verticalFactor * (0.3 + 0.7 * radialFactor);
        
        if (opacity > 0.01) {
          paint.color = const Color(0xFFE2B71B).withOpacity(opacity);
